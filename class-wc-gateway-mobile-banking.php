...
// Admin fields for the settings page
 ...
 // Separator for Payment Method Settings
        ...
        'payer_transaction_separator' => array(
            'type' => 'title',
            'title' => __('Payer/Customer Information', 'woocommerce'), // Title for the new section
            'description' => '<hr>', // Horizontal line separator
        ),

        'payer_number_label' => array(
            'title'       => __('Sender Number Label', 'woocommerce'),
            'type'        => 'text',
            'description' => __('Enter the label for the Payer Number field.', 'woocommerce'),
            'default'     => __('Customer Number:', 'woocommerce'),
            'desc_tip'    => true,
        ),
        'transaction_id_label' => array(
            'title'       => __('Payment Last 4 Digit Label ', 'woocommerce'),
            'type'        => 'text',
            'description' => __('Enter the label for the Transaction ID field.', 'woocommerce'),
            'default'     => __('Last 4 Digit of Payment Number:', 'woocommerce'),
            'desc_tip'    => true,
        ),
...
'partial_payment_label' => array(
	...
	'bkash_button_enabled' => array(
    'title'       => __('Show bKash/Nagad - Buy Now Button on Product Page', 'woocommerce'),
    'type'        => 'checkbox',
    'label'       => __('Enable a Pop up bKash/Nagad/Rocket/Upay payment button on single product pages including Delivery Cost', 'woocommerce'),
    'default'     => 'yes',
...
<input type="text" id="transaction_id" name="transaction_id" placeholder="example: 1234" required>
...
public function process_payment($order_id) {
    ...
    $order->add_order_note(__('Payment completed using ' . $payment_method . ' with transaction ID/Pin: ' . $transaction_id . ' and customer number: ' . $payer_number, 'woocommerce'));
...
function display_mobile_banking_order_info($order) {
    ...
    if ($payer_number || $transaction_id || $payment_method) {
        ...
        echo '<p style="margin: 5px 0;"><strong>' . __('Customer Number:', 'woocommerce') . '</strong> ' . esc_html($payer_number) . '</p>';
        
        // Transaction ID in red color
        echo '<p style="margin: 5px 0; color: red;"><strong>' . __('Transaction ID/Pin:', 'woocommerce') . '</strong> ' . esc_html($transaction_id) . '</p>';
...
// Add remaining due amount to the order email
add_action('woocommerce_email_order_details', 'add_remaining_due_to_email', 20, 4);

function add_remaining_due_to_email($order, $sent_to_admin, $plain_text, $email) {
    // Check if the order has a partial payment
    $remaining_due = $order->get_meta('_partial_payment_due');

    // If there is a remaining due amount, add it to the email content
    if ($remaining_due > 0) {
        // Customize the message and display the remaining due amount
        echo '<p><strong>' . __('Remaining Due Amount:', 'woocommerce') . '</strong> ' . wc_price($remaining_due) . '</p>';
    }
}
